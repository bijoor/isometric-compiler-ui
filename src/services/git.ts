...
export const extractGitHubOwnerAndRepo = (
    repoUrl: string
): [string, string] => {
...
export async function checkRepoVisibility(repoUrl: string): Promise<boolean> {
...
export async function getAllBranches(
    ...
    while (true) {
        ...
        if (response.status === 401) {
            throw new Error(
                "Invalid or unauthorized GitHub token. Please check and try again."
            );
        }
        ...
        if (!response.ok) {
            throw new Error(
                `Error fetching branches: ${response.status} ${response.statusText}`
            );
        }
